...
namespace collection
{
    class Program
    {
        ...
        {
            Program debt = new Program();
            //debt.getEmail();
            // debt.GetMaxFiveDeptors();
            //debt.WorldWar();
            //debt.BornInWinter();
            //debt.Smile();
            //debt.Low5000();
    ...
    {
            List<Debtor> deptor = debtors.FindAll(x => x.Email.Contains("rhyta.com") || x.Email.Contains("dayrep.com"));
            foreach (var item in deptor)
            {
                Console.WriteLine(item.Email);
            }

    }
        public void GetMaxFiveDeptors()
        {
            var orderedDebt = debtors.OrderByDescending(x => x.Debt).ToList();
            for (int i = 0; i < orderedDebt.Count; i++)
            {
                if (i==5)
                {
                    break;
                }
                Console.WriteLine(orderedDebt[i]);
            }
        }
        public void WorldWar()
        {
            List<Debtor> debtor = debtors.FindAll(x => x.BirthDay > new DateTime(1937, 09, 02));
            foreach (var item in debtor)
            {
                Console.WriteLine(item.BirthDay.Date);
            }
        }
        public void BornInWinter()
        {
            List<Debtor> debtor = debtors.FindAll(x => x.BirthDay.Month == 12 || x.BirthDay.Month == 1 || x.BirthDay.Month == 2);
            foreach (var item in debtor)
            {
                Console.WriteLine(item.BirthDay.ToString());
            }

        }
        public void Smile()
        {
            List<Debtor> debtor = debtors.FindAll(x =>
              x.FullName.ToLower().Contains("s")&&
                x.FullName.ToLower().Contains("m")&&
                  x.FullName.ToLower().Contains("i")&&
                    x.FullName.ToLower().Contains("l")&&
                      x.FullName.ToLower().Contains("e")
            );
            foreach (var item in debtor)
            {
        ...
        public void Low5000()
        {
            List<Debtor> debtor = debtors.FindAll(x => x.Debt < 5000);
            foreach (var item in debtor)
            {
                Console.WriteLine(item.FullName);
            }
        ...
        {
            var total = debtors.Sum(x => x.Debt);
                Console.WriteLine(total);
           
        }
        public void GetAge26()
        {
            List<Debtor> debtor = debtors.FindAll(x => x.BirthDay.Year >1984 && x.BirthDay.Year<1994);
            foreach (var item in debtor)
            {
                Console.WriteLine(item.FullName);
            }
            //List<Debtor> debtor = debtors.FindAll(x => x.BirthDay > new DateTime());
           // DateTime dateTimeNow = DateTime.Now;
           // TimeSpan result = dateTimeNow - dateTimeNow;
        ...
        public void Phone8()
        {
            List<Debtor> debtor = debtors.FindAll(x => !x.Phone.Contains("8"));
            var total = debtor.Sum(x => x.Debt);

            foreach (var item in debtor)
            {
                var birth = item.BirthDay;
                DateTime time = DateTime.Now;
                TimeSpan result = time - birth;
                double ages = (result.TotalDays / 365);
                var age = Math.Round(ages);
                var index = item.FullName.IndexOf(".");
                string surname = item.FullName.Substring(index + 2);
                Console.WriteLine($"{age} , {surname}");
        ...
        public void NoRepeat()
        {
            ...
            List<Debtor> debtor = debtors.FindAll(x => x.Phone.Count()> 1);
            

           
            foreach (var item in debtor)
            {
                var repeat = item.Phone;
                if (repeat.Count()<1)
        ...
        public void MaxYear()
        {
            // var orderedDebt = debtors.(x => x.Debt).ToList();
            // var groups = debtors.GroupBy(x => x);
            List<Debtor>debtor = debtors.OrderByDescending(x => x.BirthDay).ToList();
            foreach (var item in debtor)
            {
                if (debtor.Count >= 1)
                {
        ...
        public void Symbol()
        {
            List<Debtor> debtor = debtors.FindAll(x => x.FullName.Count() > 18 && x.Phone.Contains("7"));
        ...
        public void PayUntilBirthday()
        {
            foreach (var item in debtors)
            {
                var debt = item.Debt;
                var month = item.BirthDay.Month;
                var day = item.BirthDay.Day;
                var nextbirthday = new DateTime(2021, month, day);
                DateTime now = DateTime.Now;
                TimeSpan time = nextbirthday - now;
                var totalmonth = time.TotalDays / 30;
                if (debt <= (totalmonth * 500))
                {
        ...
        public void DebtAverage()
        {
            var total = debtors.Sum(x => x.Debt);
            List<Debtor> debtor = debtors.FindAll(x => x.Debt > total / debtors.Count);
            var orderedDebt = debtor.OrderByDescending(x => x.Debt).ToList();
        
            for (int i = 0; i < orderedDebt.Count; i++)
            {

                Console.WriteLine($"{orderedDebt[i].Debt} {orderedDebt[i].FullName}");
                //foreach (var item in debtor)
                //{
                //    var index = item.FullName.IndexOf(".");
                //    string surname = item.FullName.Substring(index + 2);
